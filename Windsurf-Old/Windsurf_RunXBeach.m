...
if project.flag.XB == 1 %only run XBeach if need to
        ...
        if abs(270-waves.D(run_number)) < 45 && waves.Hs(run_number) > 0.1 %dont run morph model if waves are too oblique or too small %notconsidered
...
function setup_xbeach_hydro(run_number, waves, tides, winds, project, grids)
   ...
   %This is the main code which sets up relevant input files at each time for xbeach and runs the simulations
    if wave_angle>(270-45) && wave_angle<(270+45) || run_number == 1 %only run simulation if waves are not super oblique %notconsidered
