function input  = XBeachRequirements(data)
...
fid = fopen('specfilelist.txt', 'w');     
fprintf(fid, '%s\n', 'FILELIST');
fprintf(fid, '%s\n', [num2str(data.timestep.Windsurf+data.general.XB.hydro_spinup),' ', num2str(data.wave.XB.dtbc),' spectra.spec']);
fprintf(fid, '%s\n', [num2str(data.timestep.Windsurf+data.general.XB.hydro_spinup),' ', num2str(data.wave.XB.dtbc),' spectra.spec']);
fclose(fid);
...
fprintf(fid, '%s\n', 'left = 0');
fprintf(fid, '%s\n', 'right = 0');
if  data.wave.XB.nonhydrostatic ~= 1 % determine if 1D or 2D grid
    fprintf(fid, '%s\n', 'back = abs_1d');
    fprintf(fid, '%s\n', 'front = abs_1d');
end
fprintf(fid, '%s\n', 'depfile = z.dep');
fprintf(fid, '%s\n', 'xfile = x.grd');
fprintf(fid, '%s\n', 'yfile = y.grd');
fprintf(fid, '%s\n', ['nx = ', num2str(length(data.grid.X)-1)]);
fprintf(fid, '%s\n', ['ny = 0']);
fprintf(fid, '%s\n', 'vardx = 1');
fprintf(fid, '%s\n', 'posdwn = -1');
...
%Waves/Flow
fprintf(fid, '%s\n', 'lwave = 1');
fprintf(fid, '%s\n', ['dtheta = ', num2str(data.wave.XB.dtheta)]);
fprintf(fid, '%s\n', ['thetamin = ', num2str(data.wave.XB.thetamin)]);
fprintf(fid, '%s\n', ['thetamax = ', num2str(data.wave.XB.thetamax)]);
fprintf(fid, '%s\n', ['random = ', num2str(data.wave.XB.random)]);
fprintf(fid, '%s\n', ['eps = ', num2str(data.wave.XB.eps)]); %threshold depth for drying and flooding
if data.wave.XB.dtheta < 359
    fprintf(fid, '%s\n', 'single_dir = 1'); %try new key name for single wave direction
end
fprintf(fid, '%s\n', ['fw = ',num2str(data.wave.XB.fw)]);
...
%             fprintf(fid, '%s\n', ['instat = swan']);
%             xb_external_wave_spectrum(data.wave.XB.spectraFilename, project.startTime+project.Times_days(run_number))
%             fprintf(fid, '%s\n', ['dthetaS_XB = ', num2str(data.wave.XB.dthetaS_XB)]); %have to do this because SWAN was assumed cartesian and xbeach expects nautical convention, not sure this should always be zero - should recheck?
%             fprintf(fid, '%s\n', ['bcfile = specfilelist.txt']);
%         elseif data.wave.XB.nonhydrostatic ~= 1 %if stationary wave case
%            % fprintf(fid, '%WaveRs\n', ['wbctype = stat']);
%             fprintf(fid, '%s\n', ['instat = 0']);
%             fprintf(fid, '%s\n', ['bcfile = specfilelist.txt']);
%         end
if data.wave.XB.nonhydrostatic == 1 %add the non-hydrostatic correction if needed. should only not do if running stationary waves
    fprintf(fid, '%s\n', ['nonh = 1']);
end
if sed.XB.lws == 1 || data.wave.XB.nonhydrostatic == 1
    fprintf(fid, '%s\n', ['lws = 1']);
else
    fprintf(fid, '%s\n', 'lws =0');
end
if sed.XB.lwt == 1 || data.wave.XB.nonhydrostatic == 1
    fprintf(fid, '%s\n', ['lwt = 1']);
else
    fprintf(fid, '%s\n', 'lwt =0');
end
...
if data.wave.XB.nonhydrostatic == 1 %nonhydrostatic automatically accounts for assymmetry and skewness, so dont need to include variables
    ...
    if sed.XB.form == 3 %add these additional values for now, other default values chosen by model are probably OK
        ...
        fprintf(fid, '%s\n', ['fric89 = 1']);
        % fprintf(fid, '%s\n', ['turbadv = 1']);
        fprintf(fid, '%s\n', ['Tm0=', num2str(data.wave.Tp(run_number))]);
...
elseif project.XB.outputType == 0
    ...
    fprintf(fid, '%s\n', ['rugdepth = ', num2str(data.wave.XB.eps)]);
...
%     else
fprintf(fid, '%s\n', ['break = ', num2str(data.wave.XB.break)]);
%   end
fprintf(fid, '%s\n', ['gamma = ', num2str(data.wave.XB.gamma)]);
fprintf(fid, '%s\n', ['gammax = ', num2str(data.wave.XB.gammax)]);
fprintf(fid, '%s\n', ['alpha = ', num2str(data.wave.XB.alpha)]);
fprintf(fid, '%s\n', ['breakerdelay = ', num2str(data.wave.XB.breakerdelay)]);
fprintf(fid, '%s\n', ['roller = ', num2str(data.wave.XB.roller)]);
fprintf(fid, '%s\n', ['beta = ', num2str(data.wave.XB.beta)]);
fprintf(fid, '%s\n', ['rfb = ', num2str(data.wave.XB.rfb)]);
fprintf(fid, '%s\n', ['shoaldelay = ', num2str(data.wave.XB.shoaldelay)]);
fprintf(fid, '%s\n', ['n = ',num2str(data.wave.XB.n)]);
fprintf(fid, '%s\n', ['wci = ',num2str(data.wave.XB.wci)]);
fprintf(fid, '%s\n', ['hwci = ',num2str(data.wave.XB.hwci)]);
fprintf(fid, '%s\n', ['cats = ',num2str(data.wave.XB.cats)]);
fprintf(fid, '%s\n', ['taper = ',num2str(data.wave.XB.taper)]);
